    ...
    runMigrations: protectedProcedure
      .mutation(async ({ ctx }) => {
        ...
        for (const stmt of [
          ...
          // 0027: Schema consolidation
          `ALTER TABLE sentimentRecords ADD COLUMN IF NOT EXISTS geoType VARCHAR(20) NOT NULL DEFAULT 'national'`,
          `ALTER TABLE sentimentRecords ADD COLUMN IF NOT EXISTS geoCode VARCHAR(10) NOT NULL DEFAULT 'ke'`,
          `ALTER TABLE africaContentSources ADD COLUMN IF NOT EXISTS scope VARCHAR(10) NOT NULL DEFAULT 'africa'`,
          `INSERT INTO africaContentSources (name, url, type, scope, createdAt) SELECT name, url, type, 'ke', createdAt FROM kenyaContentSources WHERE url NOT IN (SELECT url FROM africaContentSources)`,
          `ALTER TABLE llmCache ADD COLUMN IF NOT EXISTS source VARCHAR(50) NULL`,
          `ALTER TABLE creatorProfiles ADD COLUMN IF NOT EXISTS onboardingCompleted TINYINT(1) NOT NULL DEFAULT 0`,
          `ALTER TABLE creatorProfiles ADD COLUMN IF NOT EXISTS niche VARCHAR(255) NULL`,
          `ALTER TABLE creatorProfiles ADD COLUMN IF NOT EXISTS primaryPlatform VARCHAR(50) NULL`,
          `ALTER TABLE creatorProfiles ADD COLUMN IF NOT EXISTS tone VARCHAR(100) NULL`,
          `ALTER TABLE creatorProfiles ADD COLUMN IF NOT EXISTS contentTopics TEXT NULL`,
          `ALTER TABLE creatorProfiles ADD COLUMN IF NOT EXISTS aiGoals TEXT NULL`,
          `ALTER TABLE creatorProfiles ADD COLUMN IF NOT EXISTS aiChallenges TEXT NULL`,
          `UPDATE creatorProfiles cp JOIN aiAssistantProfiles ap ON cp.userId = ap.userId SET cp.onboardingCompleted = ap.onboardingCompleted, cp.niche = COALESCE(cp.niche, ap.niche), cp.primaryPlatform = COALESCE(cp.primaryPlatform, ap.primaryPlatform), cp.tone = ap.tone, cp.contentTopics = ap.contentTopics, cp.aiGoals = ap.goals, cp.aiChallenges = ap.challenges WHERE ap.userId IS NOT NULL`,
        ]) {
          try {
